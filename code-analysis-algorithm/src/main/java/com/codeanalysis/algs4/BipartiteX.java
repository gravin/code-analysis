...
import com.codeanalysis.algs4.Bipartite;
...
public class BipartiteX {
    ...
    private com.codeanalysis.algs4.Queue<Integer> cycle;  // odd-length cycle
     ...
     * @param  G the graph
     */
    public BipartiteX(com.codeanalysis.algs4.Graph G) {
        isBipartite = true;
        ...
        while (!q.isEmpty()) {
            ...
            for (int w : G.adj(v)) {
                if (!marked[w]) {
                    ...
                    com.codeanalysis.algs4.Stack<Integer> stack = new Stack<Integer>();
                    int x = v, y = w;
                    while (x != y) {
                        stack.push(x);
                        cycle.enqueue(y);
    ...
    private boolean check(com.codeanalysis.algs4.Graph G) {
        // graph is bipartite
        if (isBipartite) {
            for (int v = 0; v < G.V(); v++) {
                for (int w : G.adj(v)) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < F; i++) {
            int v = com.codeanalysis.algs4.StdRandom.uniform(V1 + V2);
        ...
        if (b.isBipartite()) {
            com.codeanalysis.algs4.StdOut.println("Graph is bipartite");
            for (int v = 0; v < G.V(); v++) {
                com.codeanalysis.algs4.StdOut.println(v + ": " + b.color(v));
            }
        }
        else {
            com.codeanalysis.algs4.StdOut.print("Graph has an odd-length cycle: ");
