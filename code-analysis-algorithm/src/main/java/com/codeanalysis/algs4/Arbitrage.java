...
import com.codeanalysis.algs4.BellmanFordSP;
...
public class Arbitrage {
    ...
    public static void main(String[] args) {
        ...
        // V currencies
        int V = com.codeanalysis.algs4.StdIn.readInt();
        String[] name = new String[V];

        // create complete network
        EdgeWeightedDigraph G = new EdgeWeightedDigraph(V);
        for (int v = 0; v < V; v++) {
            name[v] = com.codeanalysis.algs4.StdIn.readString();
            for (int w = 0; w < V; w++) {
        ...
        if (spt.hasNegativeCycle()) {
            ...
            for (DirectedEdge e : spt.negativeCycle()) {
                com.codeanalysis.algs4.StdOut.printf("%10.5f %s ", stake, name[e.from()]);
                stake *= Math.exp(-e.weight());
                com.codeanalysis.algs4.StdOut.printf("= %10.5f %s\n", stake, name[e.to()]);
            }
        }
        else {
