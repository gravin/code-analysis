...
public class FileIndex { 
    ...
    public static void main(String[] args) {
        ...
        com.codeanalysis.algs4.ST<String, com.codeanalysis.algs4.SET<File>> st = new ST<String, com.codeanalysis.algs4.SET<File>>();

        // create inverted index of all files
        com.codeanalysis.algs4.StdOut.println("Indexing files");
        for (String filename : args) {
            com.codeanalysis.algs4.StdOut.println("  " + filename);
            File file = new File(filename);
            com.codeanalysis.algs4.In in = new In(file);
            while (!in.isEmpty()) {
                String word = in.readString();
                if (!st.contains(word)) st.put(word, new com.codeanalysis.algs4.SET<File>());
                com.codeanalysis.algs4.SET<File> set = st.get(word);
                set.add(file);
            }
        }
