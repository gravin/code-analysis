...
import com.codeanalysis.algs4.Point2D;
...
public class GrahamScan {
    private com.codeanalysis.algs4.Stack<com.codeanalysis.algs4.Point2D> hull = new com.codeanalysis.algs4.Stack<com.codeanalysis.algs4.Point2D>();
     ...
     */
    public GrahamScan(com.codeanalysis.algs4.Point2D[] points) {
        if (points == null) throw new IllegalArgumentException("argument is null");
        if (points.length == 0) throw new IllegalArgumentException("array is of length 0");

        // defensive copy
        int n = points.length;
        com.codeanalysis.algs4.Point2D[] a = new com.codeanalysis.algs4.Point2D[n];
        for (int i = 0; i < n; i++) {
            if (points[i] == null)
        ...
        int k2;
        for (k2 = k1+1; k2 < n; k2++)
            if (com.codeanalysis.algs4.Point2D.ccw(a[0], a[k1], a[k2]) != 0) break;
        hull.push(a[k2-1]);    // a[k2-1] is second extreme point

        // Graham scan; note that a[n-1] is extreme point different from a[0]
        for (int i = k2; i < n; i++) {
            com.codeanalysis.algs4.Point2D top = hull.pop();
            while (com.codeanalysis.algs4.Point2D.ccw(hull.peek(), top, a[i]) <= 0) {
                top = hull.pop();
            }
            hull.push(top);
            hull.push(a[i]);
     ...
     */
    public Iterable<com.codeanalysis.algs4.Point2D> hull() {
        com.codeanalysis.algs4.Stack<com.codeanalysis.algs4.Point2D> s = new Stack<com.codeanalysis.algs4.Point2D>();
        for (com.codeanalysis.algs4.Point2D p : hull) s.push(p);
        return s;
    }
    ...
    private boolean isConvex() {
        ...
        com.codeanalysis.algs4.Point2D[] points = new com.codeanalysis.algs4.Point2D[n];
        int k = 0;
        for (com.codeanalysis.algs4.Point2D p : hull()) {
            points[k++] = p;
        }

        for (int i = 0; i < n; i++) {
            if (com.codeanalysis.algs4.Point2D.ccw(points[i], points[(i+1) % n], points[(i+2) % n]) <= 0) {
                return false;
            }
        }
        return true;
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        com.codeanalysis.algs4.Point2D[] points = new com.codeanalysis.algs4.Point2D[n];
        for (int i = 0; i < n; i++) {
            int x = com.codeanalysis.algs4.StdIn.readInt();
            int y = StdIn.readInt();
            points[i] = new com.codeanalysis.algs4.Point2D(x, y);
        }
