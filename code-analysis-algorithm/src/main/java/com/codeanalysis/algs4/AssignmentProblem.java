...
public class AssignmentProblem {
    ...
    private void augment() {
        ...
        for (int i = 0; i < n; i++) {
            if (xy[i] == UNMATCHED)
                G.addEdge(new com.codeanalysis.algs4.DirectedEdge(s, i, 0.0));
        }
        for (int j = 0; j < n; j++) {
            if (yx[j] == UNMATCHED)
                G.addEdge(new com.codeanalysis.algs4.DirectedEdge(n+j, t, py[j]));
        }
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (xy[i] == j) G.addEdge(new com.codeanalysis.algs4.DirectedEdge(n+j, i, 0.0));
                else            G.addEdge(new com.codeanalysis.algs4.DirectedEdge(i, n+j, reducedCost(i, j)));
            }
        }

        // compute shortest path from s to every other vertex
        com.codeanalysis.algs4.DijkstraSP spt = new DijkstraSP(G, s);
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (reducedCost(i, j) < 0) {
                    com.codeanalysis.algs4.StdOut.println("Dual variables are not feasible");
                    return false;
                }
            }
        }
    ...
    private boolean isComplementarySlack() {
        ...
        for (int i = 0; i < n; i++) {
            if ((xy[i] != UNMATCHED) && (reducedCost(i, xy[i]) != 0)) {
                com.codeanalysis.algs4.StdOut.println("Primal and dual variables are not complementary slack");
                return false;
            }
        }
        return true;
    ...
    private boolean isPerfectMatching() {
        ...
        for (int i = 0; i < n; i++) {
            if (perm[xy[i]]) {
                com.codeanalysis.algs4.StdOut.println("Not a perfect matching");
                return false;
            }
            perm[xy[i]] = true;
        }
        ...
        for (int j = 0; j < n; j++) {
            if (xy[yx[j]] != j) {
                com.codeanalysis.algs4.StdOut.println("xy[] and yx[] are not inverses");
                return false;
            }
        }
        for (int i = 0; i < n; i++) {
    ...
    public static void main(String[] args) {
        ...
        com.codeanalysis.algs4.StdOut.printf("weight = %.0f\n", assignment.weight());
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (j == assignment.sol(i))
                    com.codeanalysis.algs4.StdOut.printf("*%.0f ", weight[i][j]);
                else
                    com.codeanalysis.algs4.StdOut.printf(" %.0f ", weight[i][j]);
            }
            StdOut.println();
