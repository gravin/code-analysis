...
import com.codeanalysis.algs4.GaussianElimination;
...
public class GaussJordanElimination {
    ...
    private void show() {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                com.codeanalysis.algs4.StdOut.printf("%8.3f ", a[i][j]);
            }
            com.codeanalysis.algs4.StdOut.printf("| ");
            for (int j = n; j < n+n; j++) {
                com.codeanalysis.algs4.StdOut.printf("%8.3f ", a[i][j]);
            }
            com.codeanalysis.algs4.StdOut.printf("| %8.3f\n", a[i][n+n]);
        }
        ...
        if (isFeasible()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (Math.abs(sum - b[i]) > EPSILON) {
                    com.codeanalysis.algs4.StdOut.println("not feasible");
                    com.codeanalysis.algs4.StdOut.printf("b[%d] = %8.3f, sum = %8.3f\n", i, b[i], sum);
                    return false;
                }
            }
            return true;
        ...
        // or that yA = 0, yb != 0
            ...
            for (int j = 0; j < n; j++) {
                ...
                if (Math.abs(sum) > EPSILON) {
                    com.codeanalysis.algs4.StdOut.println("invalid certificate of infeasibility");
                    com.codeanalysis.algs4.StdOut.printf("sum = %8.3f\n", sum);
                    return false;
                }
            }
            double sum = 0.0;
            ...
            if (Math.abs(sum) < EPSILON) {
                com.codeanalysis.algs4.StdOut.println("invalid certificate of infeasibility");
                com.codeanalysis.algs4.StdOut.printf("yb  = %8.3f\n", sum);
                return false;
            }
            return true;
        }
    ...
    private static void test(String name, double[][] A, double[] b) {
        com.codeanalysis.algs4.StdOut.println("----------------------------------------------------");
        com.codeanalysis.algs4.StdOut.println(name);
        com.codeanalysis.algs4.StdOut.println("----------------------------------------------------");
        GaussJordanElimination gaussian = new GaussJordanElimination(A, b);
        if (gaussian.isFeasible()) {
            com.codeanalysis.algs4.StdOut.println("Solution to Ax = b");
            double[] x = gaussian.primal();
            for (int i = 0; i < x.length; i++) {
                com.codeanalysis.algs4.StdOut.printf("%10.6f\n", x[i]);
            }
        }
        else {
            com.codeanalysis.algs4.StdOut.println("Certificate of infeasibility");
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                A[i][j] = com.codeanalysis.algs4.StdRandom.uniform(1000);
        ...
        for (int i = 0; i < n-1; i++)
            for (int j = 0; j < n; j++)
                A[i][j] = com.codeanalysis.algs4.StdRandom.uniform(1000);
        for (int i = 0; i < n-1; i++) {
            double alpha = com.codeanalysis.algs4.StdRandom.uniform(11) - 5.0;
            for (int j = 0; j < n; j++) {
                A[n-1][j] += alpha * A[i][j];
            }
        }
