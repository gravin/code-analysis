...
import com.codeanalysis.algs4.ST;
 ...
 *  {@link com.codeanalysis.algs4.Graph} data type, which assumes the vertex names are integers
 *  between 0 and <em>V</em> - 1.
 *  It also supports initializing a symbol graph from a file.
 *  <p>
 *  This implementation uses an {@link com.codeanalysis.algs4.ST} to map from strings to integers,
 *  an array to map from integers to strings, and a {@link com.codeanalysis.algs4.Graph} to store
...
public class SymbolGraph {
    private com.codeanalysis.algs4.ST<String, Integer> st;  // string -> index
    private String[] keys;           // index  -> string
    private com.codeanalysis.algs4.Graph graph;             // the underlying graph
    ...
    public SymbolGraph(String filename, String delimiter) {
        ...
        com.codeanalysis.algs4.In in = new com.codeanalysis.algs4.In(filename);
        ...
        // line to all others
        graph = new com.codeanalysis.algs4.Graph(st.size());
        in = new In(filename);
        while (in.hasNextLine()) {
            String[] a = in.readLine().split(delimiter);
            int v = st.get(a[0]);
     ...
     */
    @Deprecated
    public com.codeanalysis.algs4.Graph G() {
        return graph;
    }
     ...
     * @return the graph associated with the symbol graph
     */
    public com.codeanalysis.algs4.Graph graph() {
        return graph;
    }
    ...
    public static void main(String[] args) {
        ...
        while (com.codeanalysis.algs4.StdIn.hasNextLine()) {
            String source = StdIn.readLine();
            if (sg.contains(source)) {
                int s = sg.index(source);
                for (int v : graph.adj(s)) {
                    com.codeanalysis.algs4.StdOut.println("   " + sg.name(v));
                }
            }
            else {
