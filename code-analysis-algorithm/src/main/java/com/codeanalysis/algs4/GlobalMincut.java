...
import com.codeanalysis.algs4.IndexMaxPQ;
...
public class GlobalMincut {
     ...
     */
    private void makeCut(int t, com.codeanalysis.algs4.UF uf) {
    ...
    private void minCut(EdgeWeightedGraph G, int a) {
        com.codeanalysis.algs4.UF uf = new UF(G.V());
    ...
    private CutPhase minCutPhase(EdgeWeightedGraph G, boolean[] marked, CutPhase cp) {
        com.codeanalysis.algs4.IndexMaxPQ<Double> pq = new IndexMaxPQ<Double>(G.V());
        for (int v = 0; v < G.V(); v++) {
        ...
        cp.weight = 0.0;
        for (com.codeanalysis.algs4.Edge e : G.adj(cp.t)) {
            cp.weight += e.weight();
        }
        return cp;
    }
    ...
    private EdgeWeightedGraph contractEdge(EdgeWeightedGraph G, int s, int t) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (com.codeanalysis.algs4.Edge e : G.adj(v)) {
                ...
                if (v < w) {
                    if (w == t)      H.addEdge(new com.codeanalysis.algs4.Edge(v, s, e.weight()));
                    else if (v == t) H.addEdge(new com.codeanalysis.algs4.Edge(w, s, e.weight()));
                    else             H.addEdge(new com.codeanalysis.algs4.Edge(v, w, e.weight()));
                }
            }
        }
        return H;
    ...
    private boolean check(EdgeWeightedGraph G) {
        ...
        for (int s = 0, t = 1; t < G.V(); t++) {
            com.codeanalysis.algs4.FlowNetwork F = new FlowNetwork(G.V());
            for (Edge e : G.edges()) {
                int v = e.either(), w = e.other(v);
                F.addEdge(new com.codeanalysis.algs4.FlowEdge(v, w, e.weight()));
    ...
    public static void main(String[] args) {
        ...
        for (int v = 0; v < G.V(); v++) {
            if (mc.cut(v)) com.codeanalysis.algs4.StdOut.print(v + " ");
        }
