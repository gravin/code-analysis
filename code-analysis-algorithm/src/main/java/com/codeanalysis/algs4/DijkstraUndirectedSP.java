...
import com.codeanalysis.algs4.DijkstraSP;
import com.codeanalysis.algs4.Edge;
import com.codeanalysis.algs4.EdgeWeightedGraph;
import com.codeanalysis.algs4.In;
import com.codeanalysis.algs4.IndexMinPQ;
...
public class DijkstraUndirectedSP {
    ...
    private com.codeanalysis.algs4.Edge[] edgeTo;            // edgeTo[v] = last edge on shortest s->v path
    private com.codeanalysis.algs4.IndexMinPQ<Double> pq;    // priority queue of vertices
     ...
     */
    public DijkstraUndirectedSP(com.codeanalysis.algs4.EdgeWeightedGraph G, int s) {
        for (com.codeanalysis.algs4.Edge e : G.edges()) {
            if (e.weight() < 0)
                throw new IllegalArgumentException("edge " + e + " has negative weight");
        }

        distTo = new double[G.V()];
        edgeTo = new com.codeanalysis.algs4.Edge[G.V()];
        ...
        while (!pq.isEmpty()) {
            int v = pq.delMin();
            for (com.codeanalysis.algs4.Edge e : G.adj(v))
                relax(e, v);
        }
    ...
    private void relax(com.codeanalysis.algs4.Edge e, int v) {
        int w = e.other(v);
     ...
     * @throws IllegalArgumentException unless {@code 0 <= v < V}
     */
    public Iterable<com.codeanalysis.algs4.Edge> pathTo(int v) {
        validateVertex(v);
        if (!hasPathTo(v)) return null;
        com.codeanalysis.algs4.Stack<com.codeanalysis.algs4.Edge> path = new Stack<com.codeanalysis.algs4.Edge>();
        int x = v;
        for (com.codeanalysis.algs4.Edge e = edgeTo[v]; e != null; e = edgeTo[x]) {
            path.push(e);
            x = e.other(x);
        }
        return path;
    ...
    private boolean check(com.codeanalysis.algs4.EdgeWeightedGraph G, int s) {
        ...
        for (int w = 0; w < G.V(); w++) {
            ...
            com.codeanalysis.algs4.Edge e = edgeTo[w];
    ...
    public static void main(String[] args) {
        ...
        com.codeanalysis.algs4.EdgeWeightedGraph G = new EdgeWeightedGraph(in);
        ...
        for (int t = 0; t < G.V(); t++) {
            if (sp.hasPathTo(t)) {
                com.codeanalysis.algs4.StdOut.printf("%d to %d (%.2f)  ", s, t, sp.distTo(t));
